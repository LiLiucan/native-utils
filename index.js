  ...
  case 'linux':
    switch (arch) {
      ...
      case 'arm':
        if (isMusl()) {
          localFileExisted = existsSync(
            join(__dirname, 'native-utils.linux-arm-musleabihf.node')
          )
          try {
            if (localFileExisted) {
              nativeBinding = require('./native-utils.linux-arm-musleabihf.node')
            } else {
              nativeBinding = require('native-utils-linux-arm-musleabihf')
            }
          } catch (e) {
            loadError = e
          }
